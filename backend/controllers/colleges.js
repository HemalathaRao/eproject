...
// @route     PUT /api/v1/college/:collegeId/photo
// @access    Private
exports.collegePhotoUpload = asyncHandler(async (req, res, next) => {
  ...
  if (college.user.toString() !== req.user.id && req.user.role !== "admin") {
    return next(
      new ErrorResponse(
        `User ${req.params.collegeId} is not authorized to update this bootcamp`,
        401
      )
    );
  }
  ...
  file.mv(`${process.env.FILE_UPLOAD_PATH}/${file.name}`, async (err) => {
    ...
    await College.findByIdAndUpdate(req.params.collegeId, { photo: file.name });
